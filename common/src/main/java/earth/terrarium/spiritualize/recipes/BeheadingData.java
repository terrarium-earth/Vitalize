package earth.terrarium.spiritualize.recipes;

import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import com.teamresourceful.resourcefullib.common.codecs.recipes.ItemStackCodec;
import com.teamresourceful.resourcefullib.common.recipe.CodecRecipe;
import earth.terrarium.spiritualize.registry.SpiritualizeRecipes;
...
public record BeheadingData(ResourceLocation id, EntityType<?> entity, ItemStack head) implements CodecRecipe<Container> {
    ...
    public RecipeSerializer<?> getSerializer() {
        return SpiritualizeRecipes.BEHEADING_DATA_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return SpiritualizeRecipes.BEHEADING_DATA.get();
    }

    public static ItemStack getHead(ServerLevel level, EntityType<?> entity) {
        return level.getRecipeManager().getAllRecipesFor(SpiritualizeRecipes.BEHEADING_DATA.get()).stream().filter(recipe -> recipe.entity() == entity).findFirst().map(BeheadingData::head).orElse(ItemStack.EMPTY);
    }
}
