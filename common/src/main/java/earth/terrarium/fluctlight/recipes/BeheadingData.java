...
public record BeheadingData(ResourceLocation id, EntityType<?> entity, ItemStack head) implements CodecRecipe<Container> {
    ...
    public RecipeSerializer<?> getSerializer() {
        return FluctlightRecipes.BEHEADING_DATA_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return FluctlightRecipes.BEHEADING_DATA.get();
    }

    public static ItemStack getHead(ServerLevel level, EntityType<?> entity) {
        return level.getRecipeManager().getAllRecipesFor(FluctlightRecipes.BEHEADING_DATA.get()).stream().filter(recipe -> recipe.entity() == entity).findFirst().map(BeheadingData::head).orElse(ItemStack.EMPTY);
    }
}
